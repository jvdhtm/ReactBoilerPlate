...
let insertSpritemap = () => {
    ...
    fs.readFile('./src/media/spritemap.svg', 'utf8', function(err, contents) {
        spritemap = contents;


            fs.readFile('./src/themes/default/Spritemap.js', 'utf8', function (err,data) {
                if (err) {
                    return console.log(err);
                }
                if (spritemap.length) {
                    var result = data.replace(/\<div id="spritemap"\>(.*?)\<\/div\>/, '<div id="spritemap">' + spritemap + '</div>');
                    fs.writeFile('./src/themes/default/Spritemap.js', result, 'utf8', function (err) {
